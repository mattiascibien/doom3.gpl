...
struct polyhedron
{
    ...
    void discard_neighbor_info()
    {
        ...
        {
            vectorInt & ni = p[i].ni;
            for (unsigned int j = 0; j < ni.Num(); j++)
                ni[j] = -1;
        }
    }
    ...
    {
        for (unsigned int i=0; i < v.Num(); i++)
            v[i] = m * v[i];
        recompute_planes();
    }
...
polyhedron make_sv(const polyhedron & oc, idVec4 light)
{
    ...
    if (lut[index].e.Num() == 0)
    {
        ...
        for (unsigned int i=0; i < oc.p.Num(); i++)
        {
            if ((oc.p[i].plane * light) > 0)
            {
                ph.p.Append(oc.p[i]);
        ...
        for (unsigned int i = 0; i < vpg.Num(); i++)
            ph.p.Append(vpg[i]);

        ph.compute_neighbors();
        ph.v.Num(); // no need to copy this data since it'll be replaced
    }
...
void polyhedron_edges(polyhedron & a, MySegments & e)
{
    ...
    for (unsigned int i = 0; i < a.e.Num(); i++)
    {
...
void clip_segments(const polyhedron & ph, MySegments & is, MySegments & os)
{
    ...
    for (unsigned int i = 0; i < is.Num(); i+=2)
    {
        idVec4 a = is[i  ];
        idVec4 b = is[i+1];
        idVec4 c;
        ...
        for (unsigned int j = 0; j < p.Num(); j++)
        {
...
{
    for (unsigned int i = 0; i < p.e.Num(); i++)
    {
...
{
    for (unsigned int i = 0; i < s.Num(); i+=2)
    {
                                       ...
                                       const viewDef_t * viewDef)
{
    ...
    idBounds outbounds;
    outbounds.Clear();
    for (unsigned int i = 0; i < out_segs.Num(); i++)
    {
